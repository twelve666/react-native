...
function Football(props) {
  ...
  function goto() {
    // navigation.navigate('CompetitionDetail', { id: data.id, match_id: data.matchEventId });
    console.log('详情');
  }
          ...
          <TouchableOpacity activeOpacity={theme.clickOpacity} onPress={goto} style={cstyle.flex1}>
            ...
            <View style={[cstyle.flexDirecR, cstyle.flexAiC, cstyle.flexJcSb, cstyle.mgT10]} >
              ...
              <View style={[cstyle.flexDirecR, cstyle.flexAiC, cstyle.flexJcC, cstyle.flex1]} onClick={() => goto()}>
                ...
                {status != MatchStaus.DELAY && <Text style={(status === MatchStaus.IN_PLAY) ? [cstyle.fwB, styles.txtBlue, styles.layoutMidWp] : ([cstyle.fwB, styles.txtRed, styles.layoutMidWp])}>{data.homeScore ? data.homeScore : 0}-{data.awayScore ? data.awayScore : 0}</Text>}
