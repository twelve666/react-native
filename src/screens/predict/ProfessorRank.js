...
class ProfessorRank extends React.Component {
  ...
  navBarTabsHandler = (props) => {
    this.setState({
      ...
      console.log(111);
      this.handleNav(this.state.isRecommend ? apprecord : appstraightwins, { sportType: this.state.sportType, page: 0, size: 15 });
    });
  ...
  tabHandler = (tabs, index) => {
    ...
    // console.log(222)
    this.setState({
      isRecommend: tabs.title === '推荐' ? true : false
    });
  ...
  handleQuarter(lang) {
    ...
    this.setState({ quarter: lang }, () => {
      let query = {
        ...
        page: 0,
        size: 15,
        quarter: Number(this.state.quarter)
      };
      console.log(3333);
